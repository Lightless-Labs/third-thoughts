...
4. **PII writer-side check.** Before writing any Parquet file, the writer validates each `Table` against a column blocklist and a value-length cap:
   - **Blocklisted column names** (case-insensitive, substring match): `path`, `file_path`, `filepath`, `cwd`, `text`, `content`, `message`, `filename`, `raw_*`, `body`, `prompt`, `source`.
...
4. Allocates an interpretation slug `<YYYY-MM-DD-HH-MM>-<runner-slug>-<uuidv7>` and a **temp sibling directory** `<output-root>/.tmp-<uuidv7>/` under the XDG interpretation root.
...
5. Atomic write: allocate temp dir at `<XDG>/com.../interpretation-failures/<analysis-run-slug>/.tmp-<uuidv7>/`, build everything inside it, on success `rename` to the final interpretation path; on failure `rename` to `<XDG>/com.../interpretation-failures/<analysis-run-slug>/<slug>/`.
