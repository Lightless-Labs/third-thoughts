...
4. **PII writer-side check.** Before writing any Parquet file, the writer validates each `Table` against a tokenised column blocklist, a type-consistency check, and a value-length cap:
   ...
   - **Blocklisted tokens** (exact match against any token in the set): `body`, `content`, `cwd`, `excerpt`, `filepath`, `prompt`, `raw`, `snippet`. *(Trimmed from 16 to 8 tokens during implementation — removed overly broad tokens like `path`, `text`, `message` that blocked legitimate analytical column names like `user_messages` and `text_length`. See `docs/solutions/best-practices/pii-blocklist-token-granularity-20260410.md`.)*
   - **Examples:** `raw_data` → tokens `{raw, data}` → `raw` matches → **blocked**. `user_messages` → tokens `{user, messages}` → no match → **passes**. `context_length` → tokens `{context, length}` → no match → **passes**. `response_body` → tokens `{response, body}` → `body` matches → **blocked**. `n_turns` → tokens `{n, turns}` → no match → **passes**.
...
53. **`export` refuses top-level split runs.** Given a split analysis run (top-level `manifest.json` has non-empty `strata`), `middens export --analysis-dir <run>` exits non-zero with a message directing the user to pass `<run>/interactive` or `<run>/subagent`. No partial notebook output. (Same contract as scenario 52, but for `export`.)
