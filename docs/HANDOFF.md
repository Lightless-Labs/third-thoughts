...
**Public corpus results website pipeline Phases 1–5 first cut are wired** (Phases 1–2/4 2026-05-29; Phase 3/5 2026-05-31). Publishing/operator docs live in `docs/public-results-publishing.md`, and `AGENTS.md` has the short future-session publishing note. Goal remains: publish selected public-HF corpus results to the website, with deterministic per-corpus metrics, optional per-corpus interpretation, comparative metrics/interpretation, and fingerprint-based rerun/reuse when corpus/process/prompt/model inputs change. Phase 1: `scripts/extract_public_corpus_metrics.py` emits public-safe `site-data/corpora/<id>/` bundles (`corpus.json`, sanitized manifests, `metrics.json`, `fingerprints.json`, `status.json`, optional `interpretation.{md,json}`) from flat+split middens runs using an allowlist documented at `docs/solutions/methodology/public-results-metrics-allowlist-20260529.md`. Phase 2: `scripts/build_public_results_site.py` renders those bundles into static pages (`index`, corpus index/details, comparative including optional interpretation, methodology with fingerprint table, downloads) and `.github/workflows/hf-corpus-analysis.yml` builds the site in PRs and deploys generated output to the existing `www` branch on trusted non-PR runs. Deterministic comparative layer: `scripts/build_public_comparative_metrics.py` writes `site-data/comparative/{corpus-index,comparative-metrics,technique-status-matrix,finding-replication-matrix}.json`; methodology note: `docs/solutions/methodology/public-comparative-metrics-20260529.md`; tests: `tests/test_extract_public_corpus_metrics.py`, `tests/test_build_public_results_site.py`, `tests/test_build_public_comparative_metrics.py`, `tests/test_public_results_fingerprint.py`, `tests/test_interpret_public_corpus.py`, and `tests/test_interpret_public_comparison.py`. Phase 5: dedicated plan `docs/plans/2026-05-31-public-results-fingerprint-invalidation-plan.md`; `scripts/public_results_fingerprint.py` computes public-safe corpus/process/interpretation fingerprints; `scripts/public_results_changed.py` compares current vs previous bundle fingerprints; trusted non-PR workflow runs fetch the prior generated `www` branch bundle from `downloads/corpora/<id>/` and reuse it when corpus+process fingerprints match; manual dispatch has `force=true` to rerun analysis anyway. Phase 3: dedicated plan `docs/plans/2026-05-31-public-results-per-corpus-interpretation-plan.md`; prompt `docs/prompts/public-corpus-interpretation-v1.md`; `scripts/interpret_public_corpus.py` reads only curated `metrics.json`, `analysis-manifest.json`, and `split-manifest.json`, writes `interpretation.md` + `interpretation.json`, updates `fingerprints.json`, rejects obvious raw/private markers, and skips model calls when the interpretation fingerprint is unchanged. CI runs it only on trusted non-PR events when repo variables `PUBLIC_RESULTS_INTERPRET_MODEL` and `PUBLIC_RESULTS_INTERPRET_COMMAND` are set. Comparative LLM interpretation first cut: plan `docs/plans/2026-05-31-public-results-comparative-interpretation-plan.md`; prompt `docs/prompts/public-comparative-interpretation-v1.md`; `scripts/interpret_public_comparison.py` reads only `site-data/comparative` allowlist JSON, writes `interpretation.md` + `interpretation.json`, rejects obvious raw/private markers, and skips unchanged comparative interpretation fingerprints. CI runs it only when `PUBLIC_RESULTS_COMPARATIVE_INTERPRET_MODEL` and `PUBLIC_RESULTS_COMPARATIVE_INTERPRET_COMMAND` are set. Prompt review is now complete: Codex found two comparative-template gaps (per-corpus split counts and distinct `0`/missing/`redacted` handling), the comparative prompt was tightened, and re-review approved both prompts with no P1/P2 blockers; review note lives at `docs/reviews/2026-06-01-public-results-interpretation-prompts-codex-review.md`. Local provider-backed smoke is also complete: using Codex CLI as the stdin/stdout runner against copied `.tmp/site-data-all` bundles, `scripts/interpret_public_corpus.py` generated per-corpus interpretation for `agent-sessions-list-mixed`, `scripts/interpret_public_comparison.py` generated comparative interpretation, `scripts/build_public_results_site.py` rebuilt `.tmp/provider-smoke/site-out`, and privacy grep passed. Registry publish/interpret controls are now wired too: `docs/corpora/public-hf-analysis-corpora.json` has explicit `publish_enabled` and `interpret_enabled`, `scripts/hf_corpus_matrix.py` filters out non-publishable corpora and passes `interpret_enabled`, and the workflow gates per-corpus LLM interpretation per corpus (plus fetches prior `interpretation.json` only when interpretation is enabled). Trusted GitHub smoke is now green. First smoke run `26980377366` got through matrix selection, materialization, full analysis, split smoke, export, and public-safe extraction for `agent-sessions-list-mixed`; build-site then exposed an artifact layout bug because downloaded artifact paths are rooted at `site-data/...`, not `.tmp/site-data/...`. The workflow collect step was fixed to search for `*/site-data/corpora/*/metrics.json`. Follow-up smoke run `26980555426` passed end-to-end: analysis succeeded, public-safe site-data collected, comparative metrics built, generated site uploaded, and the workflow deployed to `www` as `197debe` (`Update public results site`). Downloaded site artifact privacy grep passed. Still pending: provider-backed GitHub interpretation smoke because repo variables are not configured locally (`gh variable list` returned empty). Local deterministic smoke generated `.tmp/site-data-all/corpora/<id>/` for all six analysis-enabled public-HF corpora, built comparative JSON, rebuilt `.tmp/site-out-test`, and passed privacy grep. Normal publication command: `gh workflow run hf-corpus-analysis.yml -f tier=full -f corpus=all -f registry_repo= -f registry_revision=main -f force=false`. Umbrella todo remains `todos/public-results-website-pipeline.md`; next best step is configure repo interpretation variables or publish a full run without interpretation.
...
Last targeted public-results validation: 2026-06-01, `python3 -m unittest tests.test_hf_corpus_matrix tests.test_public_results_fingerprint tests.test_extract_public_corpus_metrics tests.test_interpret_public_corpus tests.test_interpret_public_comparison` → 14/14 tests passed; `python3 scripts/hf_corpus_matrix.py --tier smoke` and `--tier full` emitted expected publish/interpret flags; `actionlint .github/workflows/hf-corpus-analysis.yml` passed after artifact-collection fix. Local provider smoke: `interpret_public_corpus.py` and `interpret_public_comparison.py` with Codex CLI runner over copied public-safe site-data, followed by `build_public_results_site.py` and privacy grep over `.tmp/provider-smoke/site-out` → passed. Trusted GitHub smoke `26980555426` passed and deployed `www@197debe`; downloaded `public-results-site` artifact privacy grep passed.
