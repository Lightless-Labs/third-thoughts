...
**Public corpus results website pipeline Phases 1–2 plus Phase 4 deterministic comparative metrics are complete and documented; Phase 5 analysis-reuse is started** (Phases 1–2/4 2026-05-29; Phase 5 partial 2026-05-31). Publishing/operator docs live in `docs/public-results-publishing.md`, and `AGENTS.md` has the short future-session publishing note. Goal remains: publish selected public-HF corpus results to the website, with deterministic per-corpus metrics, optional per-corpus interpretation, comparative metrics/interpretation, and fingerprint-based rerun/reuse when corpus/process/prompt/model inputs change. Phase 1: `scripts/extract_public_corpus_metrics.py` emits public-safe `site-data/corpora/<id>/` bundles (`corpus.json`, sanitized manifests, `metrics.json`, `fingerprints.json`, `status.json`) from flat+split middens runs using an allowlist documented at `docs/solutions/methodology/public-results-metrics-allowlist-20260529.md`. Phase 2: `scripts/build_public_results_site.py` renders those bundles into static pages (`index`, corpus index/details, comparative, methodology with fingerprint table, downloads) and `.github/workflows/hf-corpus-analysis.yml` builds the site in PRs and deploys generated output to the existing `www` branch on trusted non-PR runs. Deterministic comparative layer: `scripts/build_public_comparative_metrics.py` writes `site-data/comparative/{corpus-index,comparative-metrics,technique-status-matrix,finding-replication-matrix}.json`; methodology note: `docs/solutions/methodology/public-comparative-metrics-20260529.md`; tests: `tests/test_extract_public_corpus_metrics.py`, `tests/test_build_public_results_site.py`, `tests/test_build_public_comparative_metrics.py`, and `tests/test_public_results_fingerprint.py`. Phase 5 partial: dedicated plan `docs/plans/2026-05-31-public-results-fingerprint-invalidation-plan.md`; `scripts/public_results_fingerprint.py` computes public-safe corpus/process/interpretation fingerprints; `scripts/public_results_changed.py` compares current vs previous bundle fingerprints; trusted non-PR workflow runs fetch the prior generated `www` branch bundle from `downloads/corpora/<id>/` and reuse it when corpus+process fingerprints match; manual dispatch now has `force=true` to rerun anyway. Interpretation fingerprints are present but LLM interpretation reuse remains pending because interpretation itself is not wired yet. Local smoke generated `.tmp/site-data-all/corpora/<id>/` for all six analysis-enabled public-HF corpora, built comparative JSON, rebuilt `.tmp/site-out-test`, and passed privacy grep. Normal publication command: `gh workflow run hf-corpus-analysis.yml -f tier=full -f corpus=all -f registry_repo= -f registry_revision=main -f force=false`. Umbrella todo remains `todos/public-results-website-pipeline.md`; next best step is per-corpus/comparative LLM interpretation or, if staying on Phase 5, interpretation-specific skip wiring once prompts/models exist.
