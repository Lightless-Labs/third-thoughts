...
Last targeted public-results prompt validation: 2026-06-01, `python3 -m unittest tests.test_interpret_public_corpus tests.test_interpret_public_comparison` → 8/8 tests passed. Local provider smoke: `interpret_public_corpus.py` and `interpret_public_comparison.py` with Codex CLI runner over copied public-safe site-data, followed by `build_public_results_site.py` and privacy grep over `.tmp/provider-smoke/site-out` → passed.
