...
4. **Step D — two validation runs** ← **next**: source-built run vs brew-installed run on same corpus; exports must be structurally identical. Use a small public Pi coding-agent Hugging Face dataset slice for the publishable validation path; the private corpus can still be used as a non-public stress check. (`todos/distribution-validation-runs.md`)
