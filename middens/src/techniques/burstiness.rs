...
mod tests {
    ...
    #[test]
    fn perfectly_periodic_sequence() {
        // A appears at positions 0, 3, 6, 9 (every 3rd).
        // Intervals between As: 2, 2, 2 (number of other events between consecutive As).
        // sigma = 0, mu = 2, B = (0 - 2)/(0 + 2) = -1.
        let tools: Vec<&str> = vec![
            "A", "B", "C", "A", "B", "C", "A", "B", "C", "A",
        ];
        let session = session_with_tools(&tools);
        ...
        let b_val = a_row[1].as_f64().unwrap();
        assert!(
            (b_val - (-1.0)).abs() < 0.01,
            "Periodic tool should have B close to -1, got {}",
            b_val
        );
    }

    #[test]
    fn clustered_at_start_is_bursty() {
        // A appears at positions 0, 1, 2, 3 then lots of B.
        // Intervals between As: 0, 0, 0 (adjacent).
        // sigma = 0, mu = 0, all zero => maximally bursty (B = 1.0).
        let tools: Vec<&str> = vec![
            "A", "A", "A", "A", "B", "B", "B", "B", "B", "B",
        ];
        let session = session_with_tools(&tools);
        ...
        assert!(
            b_val > 0.5,
            "Clustered tool should have B close to 1, got {}",
            b_val
        );
    }

    #[test]
    fn tool_appearing_twice_has_b_but_no_m() {
        // X appears at positions 0 and 5 (only twice). One interval = 4.
        // B = -1 (single interval, sigma=0), M = null.
        let tools: Vec<&str> = vec!["X", "A", "A", "A", "A", "X"];
        let session = session_with_tools(&tools);
        let technique = Burstiness;
        let result = technique.run(&[session]).unwrap();
        ...
        let x_row = table
            ...
            .iter()
            .find(|r| r[0] == json!("X"))
            .expect("X should be in the table");

        // B should be computed (not null).
        assert!(x_row[1].is_number(), "B should be a number for 2-occurrence tool");
        // M should be null.
        assert!(x_row[2].is_null(), "M should be null for 2-occurrence tool");
    }

    #[test]
    fn tool_appearing_once_is_skipped() {
        // Y appears only once, should not be in the table.
        let tools: Vec<&str> = vec!["Y", "A", "A", "A", "A", "A"];
        let session = session_with_tools(&tools);
        let technique = Burstiness;
        let result = technique.run(&[session]).unwrap();

        let table = &result.tables[0];
        let y_row = table.rows.iter().find(|r| r[0] == json!("Y"));
        assert!(y_row.is_none(), "Tool appearing once should be skipped");
    }

    #[test]
    fn empty_sessions_produce_zero_tools() {
        let technique = Burstiness;
        let result = technique.run(&[]).unwrap();
        let tools_finding = result
            .findings
            .iter()
            .find(|f| f.label == "tools_analyzed")
            .unwrap();
        assert_eq!(tools_finding.value, json!(0));
    }

    #[test]
    fn memory_computed_for_four_plus_occurrences() {
        // A at positions 0, 2, 4, 6 => intervals [1, 1, 1].
        // 3 intervals => memory should be computed.
        let tools: Vec<&str> = vec!["A", "B", "A", "B", "A", "B", "A"];
        let session = session_with_tools(&tools);
        let technique = Burstiness;
        let result = technique.run(&[session]).unwrap();
        ...
        assert!(
            a_row[2].is_number(),
            "M should be computed for 4-occurrence tool, got {:?}",
            a_row[2]
        );
    }

    #[test]
    fn memory_not_computed_for_three_occurrences() {
        // A at positions 0, 3, 6 => 2 intervals. Need 3+ for memory.
        let tools: Vec<&str> = vec!["A", "B", "C", "A", "B", "C", "A"];
        let session = session_with_tools(&tools);
        let technique = Burstiness;
        let result = technique.run(&[session]).unwrap();
        ...
        assert!(
            a_row[2].is_null(),
            "M should be null for 3-occurrence tool (only 2 intervals), got {:?}",
            a_row[2]
        );
    }

    #[test]
    fn mixed_burstiness_sequence() {
        // Sequence with varying intervals to produce non-trivial B.
        // A at positions 0, 1, 5, 6 => intervals [0, 3, 0].
        // mu = 1.0, sigma = sqrt((1+4+1)/3) = sqrt(2) ~ 1.414
        // B = (1.414 - 1.0) / (1.414 + 1.0) ~ 0.172
        let tools: Vec<&str> = vec!["A", "A", "B", "B", "B", "A", "A"];
        let session = session_with_tools(&tools);
        let technique = Burstiness;
        let result = technique.run(&[session]).unwrap();
        ...
        assert!(
            b_val > 0.0 && b_val < 1.0,
            "Mixed burstiness should be between 0 and 1, got {}",
            b_val
        );
    }

    #[test]
    fn aggregate_burstiness_is_weighted() {
        // Two tools: A appears 10 times (periodic), B appears 3 times (bursty).
        // Aggregate should be dominated by A.
        let mut tools = Vec::new();
        for _ in 0..10 {
            tools.push("A");
            tools.push("X"); // filler
        }
        // Add 3 Bs clustered together.
        tools.push("B");
        tools.push("B");
        tools.push("B");
        ...
        let agg = result
            .findings
            .iter()
            .find(|f| f.label == "aggregate_burstiness")
            .unwrap();

        // A is periodic (B ~ -1), B is bursty (B = 1), but A has much more weight.
        let agg_val = agg.value.as_f64().unwrap();
        assert!(
            agg_val < 0.0,
            "Aggregate B should be negative (dominated by periodic A), got {}",
            agg_val
        );
    }
}
