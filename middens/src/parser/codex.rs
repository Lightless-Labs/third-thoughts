...
mod tests {
    ...
    #[test]
    fn can_parse_detects_codex_fixture() {
        let fixture = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/codex_sample.jsonl");
        if fixture.exists() {
    ...
    fn parse_codex_fixture() {
        ...
        let s = &sessions[0];
        assert_eq!(s.source_tool, SourceTool::CodexCli);
        assert_eq!(s.id, "test-codex-session-001");
        assert_eq!(s.metadata.cwd.as_deref(), Some("/tmp/test-project"));
        assert_eq!(s.metadata.version.as_deref(), Some("0.112.0"));
        assert_eq!(s.metadata.model.as_deref(), Some("gpt-5.4"));
