...
use middens::session::{
    ...
    SessionType, SourceTool, ToolCall, ToolResult,
};
...
fn make_message_varied(
    ...
    let text = match role {
        ...
        MessageRole::Assistant => format!(
            "Assistant response at turn {} with analysis and code changes",
            turn
        ),
    ...
    let (tool_calls, tool_results, thinking) = if role == MessageRole::Assistant {
        ...
        (
            ...
            vec![tr],
            Some(format!(
                "Thinking about turn {}. Let me analyze the code and decide on the best approach.",
                turn
            )),
        )
    } else {
...
fn create_session_indexed(
    id: &str,
    num_turns: usize,
    high_correction: bool,
    session_idx: usize,
) -> Session {
    let mut messages = Vec::new();
    for i in 0..num_turns {
        let is_correction = if high_correction {
            i % 3 == 0 && i > 0
        } else {
            i % 7 == 0 && i > 2
        };
        messages.push(make_message_varied(
            MessageRole::User,
            i,
            is_correction,
            session_idx,
...
#[given(
    expr = "a set of {int} sessions, each with {int}-{int} turns, including thinking and tool use"
)]
...
#[given(
    expr = "a set of {int} sessions with a mix of high and low correction rates, tool calls, and thinking"
)]
...
fn given_mixed_length_sessions(
...
fn given_invalid_path(world: &mut MiddensWorld) {
    ...
    world.file_path = Some(
        world
            ...
            .unwrap()
            .path()
            .join("nonexistent_sessions.json"),
    );
}
...
fn then_summary_mentions_two(world: &mut MiddensWorld, kw1: String, kw2: String) {
    ...
    assert!(
        summary_lower.contains(&kw1.to_lowercase()),
        "Summary missing '{}': {}",
        kw1,
        result.summary
    );
    assert!(
        summary_lower.contains(&kw2.to_lowercase()),
        "Summary missing '{}': {}",
        kw2,
        result.summary
    );
    ...
    let finding = result
        ...
        .unwrap_or_else(|| {
            panic!(
                ...
                label,
                result.findings.iter().map(|f| &f.label).collect::<Vec<_>>()
            )
        });
    assert!(
        finding.value.is_number(),
        "Finding '{}' is not numeric: {:?}",
        label,
        finding.value
    );
}
...
fn then_has_string_finding(world: &mut MiddensWorld, label: String) {
    ...
    assert!(
        finding.value.is_string(),
        "Finding '{}' is not a string: {:?}",
        label,
        finding.value
    );
}
...
fn then_has_two_findings(world: &mut MiddensWorld, label1: String, label2: String) {
    let result = world.technique_result.as_ref().expect("No result");
    assert!(
        result.findings.iter().any(|f| f.label == label1),
        "Finding '{}' not found",
        label1
    );
    assert!(
        result.findings.iter().any(|f| f.label == label2),
        "Finding '{}' not found",
        label2
    );
}
...
fn then_has_table(world: &mut MiddensWorld, name: String) {
    ...
    assert!(
        ...
        "Table '{}' not found in: {:?}",
        name,
        result.tables.iter().map(|t| &t.name).collect::<Vec<_>>(),
    );
}

#[then(
    expr = "the result summary should state that {int} sessions were skipped due to insufficient turns"
)]
fn then_summary_skipped(world: &mut MiddensWorld, count: i32) {
    let result = world.technique_result.as_ref().expect("No result");
    let summary_lower = result.summary.to_lowercase();
    assert!(
        summary_lower.contains(&format!("skipped {}", count))
            || summary_lower.contains(&format!("{} skipped", count)),
        "Summary does not mention {} skipped sessions: {}",
        count,
        result.summary,
    );
}
...
fn then_summary_zero(world: &mut MiddensWorld) {
    ...
    assert!(
        ...
        "Summary doesn't indicate 0 sessions: {}",
        result.summary,
    );
}
...
fn then_technique_fails(world: &mut MiddensWorld) {
    assert!(
        world.error.is_some(),
        "Expected failure but technique succeeded"
    );
...
fn then_stderr_not_empty(world: &mut MiddensWorld) {
    assert!(
        !world.cli_stderr.is_empty(),
        "Expected stderr output but it was empty"
    );
}
...
#[given(
    expr = "a set of {int} sessions across {int} projects, each with {int}-{int} turns, including thinking and tool use"
)]
...
#[given(
    expr = "a set of {int} sessions across {int} projects spanning {int} days with timestamps, each with {int}-{int} turns"
)]
fn given_sessions_with_projects_and_timestamps(
    ...
    world.sessions = (0..session_count)
        .map(|i| {
            ...
            let session_start =
                base + Duration::days(day_offset as i64) + Duration::minutes(i as i64);
...
fn then_no_table_cell_contains_raw_text(world: &mut MiddensWorld) {
    let result = world
        .technique_result
        .as_ref()
        .expect("Expected technique result");
    for table in &result.tables {
        for row in &table.rows {
            for cell in row {
                if let serde_json::Value::String(s) = cell {
                    assert!(
                        !s.contains("User message at turn"),
                        "Table cell contains user text: {}",
                        s
                    );
                    assert!(
                        !s.contains("Assistant response at turn"),
                        "Table cell contains assistant text: {}",
                        s
                    );
                    assert!(
                        !s.contains("Thinking about turn"),
                        "Table cell contains thinking text: {}",
                        s
                    );
                    assert!(
                        !s.contains("/home/user/workspace/"),
                        "Table cell contains cwd path: {}",
                        s
                    );
...
#[then(
    "the result summary should state that insufficient projects were found for cross-project analysis"
)]
