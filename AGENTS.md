...
## Public results website publishing

Human/operator guide: `docs/public-results-publishing.md`.

The public results site is generated by `.github/workflows/hf-corpus-analysis.yml` from public-safe `site-data`, not from raw transcripts. Normal publication is a trusted manual workflow run:
...
PRs build the site but do **not** deploy. Trusted `workflow_dispatch` / scheduled runs deploy by force-pushing generated `site-out/` to the existing `www` branch. Keep raw transcripts, raw per-session tables, notebooks, Parquet files, source paths, session ids, and tool payloads out of published output unless a future explicit allowlist says otherwise.

Local building/debugging uses:

```bash
python3 scripts/extract_public_corpus_metrics.py ...
python3 scripts/build_public_comparative_metrics.py --corpora-dir .tmp/site-data/corpora --output .tmp/site-data/comparative
python3 scripts/build_public_results_site.py --site-data .tmp/site-data --output .tmp/site-out
```

Before publishing locally generated output, run a privacy grep/tripwire as documented in `docs/public-results-publishing.md`.
